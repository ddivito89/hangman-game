...
function newGame(x, wins, losses) {
  ...
  statusDiv.innerHTML = (guessesRemaining + " guesses remaining");
  scoreDiv.innerHTML = ("Wins: " + wins + "   Losses: " + losses);
  badGuessDiv.innerHTML = ("bad guesses: ");

  //alpha validation and check game functions
  function isLetter(str) {
    return str.length === 1 && str.match(/[a-z]/i);
  }

  function checkGame(x, wins, losses) {
    document.onkeydown = function(event) {
      if (event.key === "Enter") {
        if (gameOver === true) {
          x++;
      ...
      if (isLetter(userGuess)) {
        ...
        if (letterArray.indexOf(userGuess) > -1) {
          ...
          if (guessArray.indexOf("_") === -1) {
            ...
            badGuessDiv.innerHTML = ("-- hit enter --");
            scoreDiv.innerHTML = ("Wins: " + wins + "   Losses: " + losses);
            checkGame(x, wins, losses);
          }
        } else {
